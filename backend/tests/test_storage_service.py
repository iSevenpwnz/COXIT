...
class TestStorageService:
    ...
    @patch('src.services.storage_service.settings')
    @patch('builtins.open', new_callable=mock_open)
    def test_save_metadata(self, mock_file, mock_settings, mock_load):
    ...
    def test_get_recent_history(self, mock_settings, mock_load):
        ...
        # Mock metadata with some legacy items
        metadata = [
            {
                "id": "legacy-1",
        ...
        # Should return last 2 items in reverse order (most recent first)
        assert len(result) == 2
        assert result[0].id == "new-2"
        assert result[1].id == "new-1"
        
        # Check that legacy item gets default values
